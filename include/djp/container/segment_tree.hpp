...
class segment_tree {
 ...
 private:
  ...
  struct node {
    node(pos_type b, pos_type e, size_type tp)
        : begin{b}, end{e}, tree_pos{tp} {}

    bool is_leaf() const { return begin + 1 == end; }
    pos_type middle() const { return begin + (end - begin) / 2; }
 ...
 public:
  /// Constructs the segment tree with the elements in
  /// the range [first, last) using the associative binary operator op.
