...
/// weight of the min s-t cut as its value is equal to the max-flow  from
/// \p source to \p target (See max-flow min-cut theorem).
///
...
///
/// \tparam Graph The graph type. Must model a FlowNetwork so it can be used as
...
auto min_st_cut(const Graph &graph, const size_t source, const size_t target,
  ...
  source_side.assign(graph.num_vertices(), false);
