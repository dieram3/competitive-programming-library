...
TEST(calc_lis_size, HandlesSmallRanges) {
  ...
  EXPECT_EQ(6, calc_lis_size(seq, std::less<int>{}));
}
...
TEST(calc_lis_size, HandlesLongRanges) {
  ...
  EXPECT_EQ(1, calc_lis_size(seq, std::greater<int>{}));
}

TEST(calc_lis_size, HandlesNonStrict) {
  std::vector<int> seq(4096);
  std::iota(seq.begin() + 96, seq.end(), 0);
  EXPECT_EQ(4000, calc_lis_size(seq, std::less<int>{}));
  EXPECT_EQ(4096, calc_lis_size(seq, std::less_equal<int>{}));

  std::vector<int> seq2 = {130, 150, 150, 140, 120, 100, 30, 50, 50, 5};
  EXPECT_EQ(6, calc_lis_size(seq2, std::greater<int>{}));
  EXPECT_EQ(8, calc_lis_size(seq2, std::greater_equal<int>{}));
}
